...
- 查文档时先从提供的官方文档主页或目录页进入，再沿站内链接定位具体内容；不要自行拼接或猜测文档 URL 后直接请求，避免产生大量 `404`。
- 如果官方文档站点暂时无法访问，或提供的文档链接无法打开，可改用 `context7` MCP 中的相关文档作为兜底参考，但优先级仍低于项目里已提供的官方资料与本地参考文件。
...
## 分支与合并
- 修改代码时先新建分支，在分支上完成开发；不要直接在主线分支上改动。
- 需要跨机器协作、备份进度或暂存当前修改时，可以将分支推送到远端保存中间状态。
- 功能开发或普通改动完成后，合并回主线时优先使用 `squash merge`，避免直接 `merge` 带入大量噪杂提交记录。
- 发布场景按 `docs/RELEASE.md` 执行：如果发布分支中的全部改动都符合发布提交范围，可整理为单独的发布提交后再合并回主线；如果分支里混有非发布改动，必须先拆分，不能直接通过 `squash merge` 混入发布提交。
- 发布 tag 应打在 `main` 上对应的发布提交。
- 合并完成并确认主线内容无误后，建议删除本地与远端对应分支，保持仓库整洁。
